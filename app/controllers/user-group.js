...
const createData = async (req, res) => {
	try {
        ...
        const data = await models.UserGroup.bulkCreate(req.body);
...
const getDataById = async (req, res) => {
	try {
		const { id } = req.params;
		const data = await models.UserGroup.findOne({ where: { user_group_id: id } });
...
const updateData = async (req, res) => {
	try {
		...
		if (data) {
            ...
            // update
            const updated = await models.UserGroup.update(req.body, {
                where: { user_group_id: id }
            });

			if (updated.length) {
				// success update
				const updatedData = await models.UserGroup.findOne({ where: { user_group_id: id } });
...
const deleteData = async (req, res) => {
	try {
		const { id } = req.params;
		const deleted = await models.UserGroup.destroy({ where: { user_group_id: id } });
...
const deleteManyData = async (req, res) => {
	try {
		...
		const deleted = await models.sequelize.transaction(async (t) => {
			const data = await models.UserGroup.destroy({ where: { user_group_id: id } }, { transaction: t });
...
const truncateData = async (req, res) => {
	try {
		await models.UserGroup.destroy({ truncate: true, restartIdentity: true });

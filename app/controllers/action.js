...
const createData = async (req, res) => {
	try {
		...
		const data = await models.Action.bulkCreate(req.body);
...
const getDataById = async (req, res) => {
	try {
		const { id } = req.params;
		const data = await models.Action.findOne({ where: { action_id: id } });
...
const updateData = async (req, res) => {
	try {
		...
		if (data) {
			...
			if (updated.length) {
				// success update
				const updatedData = await models.Action.findOne({ where: { action_id: id } });
...
const deleteData = async (req, res) => {
	try {
		const { id } = req.params;
		const deleted = await models.Action.destroy({ where: { action_id: id } });
...
const deleteManyData = async (req, res) => {
	try {
		...
		const deleted = await models.sequelize.transaction(async (t) => {
			const data = await models.Action.destroy({ where: { action_id: id } }, { transaction: t });
...
const truncateData = async (req, res) => {
	try {
		await models.Action.destroy({ truncate: true, restartIdentity: true });

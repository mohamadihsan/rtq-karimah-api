...
const createData = async (req, res) => {
	try {
        ...
        const data = await models.Menu.bulkCreate(req.body);
...
const getDataById = async (req, res) => {
	try {
		const { id } = req.params;
		const data = await models.Menu.findOne({ where: { menu_id: id } });
...
const updateData = async (req, res) => {
	try {
		...
		if (data) {
            ...
            // update
            const updated = await models.Menu.update(req.body, {
                where: { menu_id: id }
            });

			if (updated.length) {
				// success update
				const updatedData = await models.Menu.findOne({ where: { menu_id: id } });
...
const deleteData = async (req, res) => {
	try {
		const { id } = req.params;
		const deleted = await models.Menu.destroy({ where: { menu_id: id } });
...
const deleteManyData = async (req, res) => {
	try {
		...
		const deleted = await models.sequelize.transaction(async (t) => {
			const data = await models.Menu.destroy({ where: { menu_id: id } }, { transaction: t });
...
const truncateData = async (req, res) => {
	try {
		await models.Menu.destroy({ truncate: true, restartIdentity: true });

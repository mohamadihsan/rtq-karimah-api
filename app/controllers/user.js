...
const createData = async (req, res) => {
	try {
        ...
        const data = await models.User.bulkCreate(req.body);
...
const updateData = async (req, res) => {
	try {
		...
		if (data) {
			...
			if (updated.length) {
				// success update
				const updatedData = await models.User.findOne({ where: { user_id: id } });
...
const deleteData = async (req, res) => {
	try {
		const { id } = req.params;
		const deleted = await models.User.destroy({ where: { user_id: id } });
...
const deleteManyData = async (req, res) => {
	try {
		...
		const deleted = await models.sequelize.transaction(async (t) => {
			const data = await models.User.destroy({ where: { user_id: id } }, { transaction: t });
